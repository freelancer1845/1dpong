...
public class VelocityLogic {
    ...
    /**
     * Updates the velocities of the game objects (ATM only the ball).
     * 
     * @param gameStatus
     *            with current ball status.
     * @param deltaTime
     *            not used yet.
     */
    ...
    private static void increaseLogic(GameStatus gameStatus) {
        gameStatus.getBall()
                .setVelX((Math.abs(gameStatus.getBall().getVelX())
                        + (gameStatus.getGameSettings().getVelocityConstantIncreaseValue()))
                        * Math.signum(gameStatus.getBall().getVelX() * -1));
    }
    ...
    private static void parabolicHitPointLogic(GameStatus gameStatus) {
        ...
        if (gameStatus.getBall().getVelX() <= 0) {
            ...
            positionInDeadline = ballPosition / (GameConstants.GAME_WORLD_WIDTH + ballWidth - deadLinePosition)
                    - deadLinePosition / (GameConstants.GAME_WORLD_WIDTH + ballWidth - deadLinePosition);
            velX = -1;
        }

        velX = velX * ((min - max) * 4 * positionInDeadline * positionInDeadline + (max - min) * 4 * positionInDeadline
                + min);
    ...
    private static void linearHitPointLogic(GameStatus gameStatus) {
        ...
        if (gameStatus.getBall().getVelX() <= 0) {
            ...
            ballPosition = gameStatus.getBall().getX() + ballWidth;
            deadLinePosition = gameStatus.getRightDeadline().getX();
            positionInDeadline = ballPosition / (GameConstants.GAME_WORLD_WIDTH - deadLinePosition)
                    - deadLinePosition / (GameConstants.GAME_WORLD_WIDTH - deadLinePosition);
            velX = -1;
        }
