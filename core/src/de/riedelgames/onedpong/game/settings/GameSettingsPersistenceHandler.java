...
public class GameSettingsPersistenceHandler {
	...
	private static final String VELOCITY_DEFAULT_MOD = VelocityMod.constantIncrease.toString();
	...
	private static void loadVelocitySettings(GameSettings settings, Properties propertiesLoader) {
		...
		settings.setVelocityMod(VelocityMod.valueOf((propertiesLoader.getProperty(VELOCITY_MOD, VELOCITY_DEFAULT_MOD))));
		settings.setVelocityConstantIncreaseValue(Float.valueOf(propertiesLoader.getProperty(VELOCITY_CONSTANT_INCREASE_VALUE, VELOCITY_CONSTANT_INCREASE_DEFAULT_VALUE)));
		settings.setVelocityParabolicHitpointMin(Float.valueOf(propertiesLoader.getProperty(VELOCITY_PARABOLIC_HITPOINT_MIN, VELOCITY_PARABOLIC_HITPOINT_MIN_DEFAULT)));
		settings.setVelocityParabolicHitpointMax(Float.valueOf(propertiesLoader.getProperty(VELOCITY_PARABOLIC_HITPOINT_MAX, VELOCITY_PARABOLIC_HITPOINT_MAX_DEFAULT)));
		settings.setBallStartVelocity(Float.valueOf(propertiesLoader.getProperty(BALL_START_VELOCITY, BALL_START_VELOCITY_DEFAULT)));
	
	}
	
	private static void loadDeadlineSettings(GameSettings settings, Properties propertiesLoader) {
		
		settings.setDeadlineType(DeadlineType.valueOf(propertiesLoader.getProperty(DEADLINE_TYPE, DEADLINE_DEFAULT_TYPE)));
		settings.setDeadlineConstantDecreaseValue(Float.valueOf(propertiesLoader.getProperty(DEADLINE_CONSTANT_DECREASE_VALUE, DEADLINE_CONSTANT_DECREASE_DEFAULT_VALUE)));
	...
	private static void loadRulesSettings(GameSettings settings, Properties propertiesLoader) {
		...
		settings.setPointsToBePlayed(Integer.valueOf(propertiesLoader.getProperty(POINTS_TO_BE_PLAYED, POINTS_TO_BE_PLAYED_DEFAULT)));
	...
	private static void loadScreenSettings(GameSettings settings, Properties propertiesLoader) {
		...
		settings.setFullScreenMod(Boolean.valueOf(propertiesLoader.getProperty(RUN_IN_FULLSCREEN, RUN_IN_FULLSCREEN_DEFAULT)));
	...
	private static void saveVelocitySettings(GameSettings settings, Properties propertiesSaver) {
		...
		propertiesSaver.setProperty(VELOCITY_CONSTANT_INCREASE_VALUE, Float.toString(settings.getVelocityConstantIncreaseValue()));
		propertiesSaver.setProperty(VELOCITY_PARABOLIC_HITPOINT_MIN, Float.toString(settings.getVelocityParabolicHitpointMin()));
		propertiesSaver.setProperty(VELOCITY_PARABOLIC_HITPOINT_MAX, Float.toString(settings.getVelocityParabolicHitpointMax()));
	...
	private static void saveDeadlineSettings(GameSettings settings, Properties propertiesSaver) {
		...
		propertiesSaver.setProperty(DEADLINE_CONSTANT_DECREASE_VALUE, Float.toString(settings.getDeadlineConstantDecreaseValue()));
